...
function formViewpass() {
    document.addEventListener("click", function (e) {
        ...
        if (targetElement.closest('.toggle-pass') && targetElement.closest('[class*="__viewpass"]')) {
            e.preventDefault()
            const parentInput = targetElement.closest('[class*="__viewpass"]')
            let inputType = parentInput.classList.contains('show') ? "password" : "text";
            targetElement.closest('.toggle-pass').classList.toggle('show');
            parentInput.classList.toggle('show')
            parentInput.querySelector('input').setAttribute("type", inputType);
        }
